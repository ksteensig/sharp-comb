...
namespace MonadicParserCombinator
{
    ...
    public static class Parser
    {
        ...
        public static Parser<char> Ascii = Any.Where(result => result < 128);
        ...
        public static Parser<T> Exclude<T>(this Parser<T> parser, Parser<T> exclusion) => i =>
        {
            Input remainder = i;

            var res1 = parser(i);
            var res2 = exclusion(i);

            if (res1.IsSuccess ^ res2.IsSuccess && res1.IsSuccess)
            {
                return res1;
            }
            else
            {
                return Result<T>.Failure(i, "Could not exclusively parse either");
            }
        };
